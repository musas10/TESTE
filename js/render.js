...
const Render = {
    ...
    gerarCardHTML(aparelho) {
        let tagsHTML = (aparelho.tags && aparelho.tags.length > 0) 
            ? `<div class="tags-container">${aparelho.tags.slice(0,3).map(t => `<span class="tag-chip">${t}</span>`).join('')}</div>` 
            : '';
        ...
        return `
            <div class="card borda-${aparelho.status}">
                ...
                <div class="card-header">
                    <h2>${aparelho.id}</h2>
                    <div>
                        <button class="btn-acao" style="border:none; padding:5px; border-radius:50%; margin-right: 5px;" title="Detalhes" onclick="Modais.abrirDetalhes('${aparelho.id}')">👁️</button>
                        <button class="btn-acao" style="border:none; padding:5px; border-radius:50%; margin-right: 5px;" title="Editar" onclick="Modais.abrirModalAtivo('${aparelho.id}')">✏️</button>
                        <button class="btn-perigo" style="border:none; padding:5px; border-radius:50%;" title="Excluir" onclick="Aparelhos.excluir('${aparelho.id}')">🗑️</button>
                    </div>
                </div>
    ...
    renderizarTela(dadosFiltrados) {
        ...
        if (dadosFiltrados.length === 0) {
            painel.innerHTML = '<p style="color: #64748b; text-align: center; width: 100%; margin-top: 50px;">Nenhum ativo corresponde à busca.</p>';
            return;
        }
        ...
        if (window.visualizacaoAtual === 'cards') {
            ...
            dadosFiltrados.forEach(ap => {
                htmlTabela += `
                    <tr>
                        ...
                        <td>
                            <button class="btn-acao" onclick="Modais.abrirDetalhes('${ap.id}')">👁️</button>
                            <button class="btn-acao" onclick="Modais.abrirModalAtivo('${ap.id}')">✏️</button>
                        </td>
                    </tr>
                `;
            });
            htmlTabela += '</tbody></table>';
            painel.innerHTML = htmlTabela;
        } 
            ...
            gruposUnicos.forEach(grupoNome => {
                ...
                colunaHTML += `</div></div>`;
                painel.innerHTML += colunaHTML;
            });
        }
    },

    // Funções do Gerenciador Base (Oferta, BM, Pagina)
